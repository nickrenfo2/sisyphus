...
{ //@@@@@@@@@@@@@@@@      Web Sockets Functions    @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  var socket = require('socket.io-client')(connString, {
    query: 'serial=' + mySisbot.serial
  });
  ...
  function filterByExtension(files, ext) {
    return files.filter(function(file) {
      return file.substr(ext.length * -1) === ext
    })
  }
...
function loadPlaylist(plistName) {
  ...
  lines.forEach(function(line) {
    var parts = line.split(/[ \t\r]+/); //split on sp's and/or tabs
...
function go(ind) {
  if (NEWPLIST) {
    ...
    paused = false; //?
    NEWPLIST = false;
    console.log('go: begin index', ind);
    PLINDEX = ind || 0;
    nextPlaylistLine(PLINDEX, plLinesMax);
